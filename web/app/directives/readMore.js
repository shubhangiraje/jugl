app.directive('readMore', function() {
  return {
    ...
    link: function(scope, elem, attr, ctrl, transclude) {
      var moreText = angular.isUndefined(scope.moreText) ? ' <a class="read-more">...mehr anzeigen</a>' : ' <a class="read-more">' + scope.moreText + '</a>',
        lessText = angular.isUndefined(scope.lessText) ? ' <a class="read-less">weniger anzeigen</a>' : ' <a class="read-less">' + scope.lessText + '</a>',
        ellipsis = angular.isUndefined(scope.ellipsis) ? '' : scope.ellipsis,
        limit = angular.isUndefined(scope.limit) ? 250 : scope.limit;
      ...
      function readmore(text_read_more) {

        var text = text_read_more,
          orig = text,
          regex = /\s+/gi,
