    ...
    multiselect.directive('multiselect', ['$filter', '$document', '$log','$timeout', function ($filter, $document, $log,$timeout) {
        return {
            ...
            link: function ($scope, $element, $attrs, $ngModelCtrl) {
                ...
                $scope.searchLimit = $scope.searchLimit || 250;
                ...
                $scope.toggleItem = function (item) {
					...
					else if($scope.selectionLimit ===1){
						$scope.unselectedOptions=[];
						$scope.selectedOptions.push(item);
						$scope.selectedOptions.splice(unselectedIndex,1);
						$scope.unselectedOptions.push(item);
					}
                };
...
angular.module("multiselect.html", []).run(["$templateCache", function ($templateCache) {
  $templateCache.put("multiselect.html",
    ...
    "                {{getDisplay(option)}}<span class=\"flag flag-32 flag-{{option.flag}}\"></span>\n" +
    "            </a>\n" +
    "        </li>\n" +
    ...
    "                {{getDisplay(option)}}\n" +
    "            <span class=\"flag flag-32 flag-{{option.flag}}\"></span></a>\n" +
    "        </li>\n" +
    "\n" +
