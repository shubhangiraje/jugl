...
<div class="row">
    <div class="col-sm-12">
        <?php $form = ActiveForm::begin([
            'id'=>'search-request-validation-accept-from'
        ]); ?>

        <?= $form->field($model, 'default_text_id')->widget(\kartik\select2\Select2::classname(), [
            'data' => \app\models\DefaultText::getDefaultTextList(\app\models\DefaultText::SEARCH_REQUEST_VALIDATION_ACCEPTED),
            'options' => [
                'placeholder' => '',
                'id'=>'search-request-validation-accept-select',
                'onchange'=>'
                    var data = $(this).select2("data");
                    var text = data[0].text;                   
                    $("#search-request-validation-accept-from").find("#defaulttext-default_text_edit").val(text);
                '
            ],
            'hideSearch' => true,
        ]) ?>
...
$('#search-request-validation-accept-from').on('beforeSubmit', function(e) {
    var form = $(this);
