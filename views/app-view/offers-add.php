...
<div class="searches offers-add">
    <div class="container">
            ...
            <div class="searches-details-fields">
                <div class="searches-fields-box fields-full">
                    ...
                    <div class="field-box clearfix fields-full field-scheduled">
                        ...
                        <div class="fieldin-item">
                            <div class="field-box-entry" bs-has-classes>
                                <input type="checkbox" ng-true-value="1" ng-false-value="0" i-check ng-model="offer.is_active_immediately" server-error="offer.$errors.is_active_immediately">
                                <label><?= Yii::t('app','Ab sofort') ?></label>
                            </div>
                        </div>

                        <div ng-if="!offer.is_active_immediately" class="fields-scheduled-dt-box">
                            <label><?= Yii::t('app', 'Ab dem') ?>:</label>

                            <div class="scheduled-dt-box">
                                <div class="scheduled-date-box">
                                    <div class="field-box-select" dropdown-toggle select-click bs-has-classes>
                                        <select ng-model="offer.scheduled_dt_parts.day" selectpicker="{title:''}" ng-options="item.key as item.val for item in birthDayList">
                                            <option value=""><?= Yii::t('app', 'Day'); ?></option>
                                        </select>
                                    </div>
                                    <div class="field-box-select" dropdown-toggle select-click bs-has-classes>
                                        <select ng-model="offer.scheduled_dt_parts.month" selectpicker="{title:''}" ng-options="item.key as item.val for item in birthMonthList">
                                            <option value=""><?= Yii::t('app', 'Monat'); ?></option>
                                        </select>
                                    </div>
                                    <div class="field-box-select" dropdown-toggle select-click bs-has-classes>
                                        <select ng-model="offer.scheduled_dt_parts.year" selectpicker="{title:''}" ng-options="item.key as item.val for item in birthYearList">
                                ...
                                <div class="scheduled-time-box">
                                    <div class="field-box-select field-box-select-hours" dropdown-toggle select-click bs-has-classes>
                                        <select ng-model="offer.scheduled_dt_parts.hours" selectpicker="{title:''}" ng-options="item.key as item.val for item in hoursList">
                                            <option value=""><?= Yii::t('app', 'Stunde'); ?></option>
                                        </select>
                                    </div>
                                    <div class="field-box-select" dropdown-toggle select-click bs-has-classes>
                                        <select ng-model="offer.scheduled_dt_parts.minutes" selectpicker="{title:''}" ng-options="item.key as item.val for item in minutesList">
						...
						<div class="fieldin-item">
							<div class="field-box-entry entry-full-block">
								<div class="field-box-textarea" bs-has-classes>
									<textarea ng-model="offer.description" maxlength="4000" server-error="offer.$errors.description" placeholder="<?=Offer::getEncodedAttributeLabel('description')?> eingeben *"></textarea>
								</div>
							</div>
						</div>
                    </div>
        ...
        <h4 class="text-center"><?= Yii::t('app', 'Die Vorschau ermöglicht es Dir, Deine Anzeige vor der Veröffentlichung noch einmal genau zu überprüfen und ggf. Korrekturen vorzunehmen.')?></h4>

        <?php /*<button ng-click="offerAddCtrl.saveDraft()">saveDraft</button>*/ ?>
