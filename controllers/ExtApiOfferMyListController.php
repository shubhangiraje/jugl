...
class ExtApiOfferMyListController extends \app\components\ExtApiController {
...
//            }
            ...
            if (count($model->files)>0) {
                $idata['image']=$model->files[0]->getThumbUrl('offerMobile');
            } else {
            ...
            foreach($model->offerActiveRequests as $offer) {
                ...
                if ($idata['bestBet']<$offer->bet_price && !$odata['isExpired'] && $offer->status==\app\models\OfferRequest::STATUS_ACTIVE) {
    ...
    public function actionGetOfferViewUsers($offer_id) {
        ...
        foreach ($offer_view_users as $offer_view_user) {
            $data[] = ['avatarMobile'=>$offer_view_user->user->getAvatarThumbUrl('avatarMobile')];
        }
