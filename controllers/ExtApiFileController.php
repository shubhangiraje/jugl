...
class ExtApiFileController extends \app\components\ExtApiController {
    ...
    private function upload($file) {
        if ($file && ! $file->getHasError()) {
            ...
            if (preg_match('%^mov$%i',$ext) || preg_match('%^VID.*\.3gp$%',$file->name)) {
                ...
                if ($result) {
                    \Yii::error("failed converting $file->tempName to mp4, result: $result, output:\n" . join("\n", $output));
            ...
            } elseif (preg_match('%^(m4a|3gp)$%i',$ext)) {
                // repack to mp3 container
    ...
    public function actionUpload()
    {
        ...
        if (is_string($model)) {
            ...
            $data['thumbs']['imageBig']=$model->getThumbUrl('imageBig');
            $data['thumbs']['avatarMobile']=$model->getThumbUrl('avatarMobile');
            $data['thumbs']['trollboxSmall']=$model->getThumbUrl('trollboxSmall');
            $data['thumbs']['infoCommentSmall']=$model->getThumbUrl('infoCommentSmall');
            $data['thumbs']['fancybox']=$model->getThumbUrl('fancybox');
