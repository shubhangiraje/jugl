...
class ExtApiSearchRequestController extends \app\components\ExtApiController {
...
//                }
		...
		return ['result'=>true, 'searchRequest'=>$searchRequest,'willBeValidated'=>$searchRequest->status==\app\models\SearchRequest::STATUS_AWAITING_VALIDATION];
    }

    public function getComments($searchRequestId, $pageNum=1) {
    ...
    public function actionDetails($id) {
		...
		if($model->search_request_type==SearchRequest::SEARCH_REQUEST_TYPE_EXTERNAL_AD){		
			...
			$providerModel=\app\models\AdvertisingSearchRequestProvider::find()->where('id=:id',['id'=>$model->provider_id])->one();
        ...
        if (empty($data['images'])) {
            $data['images']=[\app\components\Thumb::createUrl('/static/images/account/default_interest.png','searchRequest',true)];
        }
    ...
    public function actionListComments($searchRequestId, $pageNum) {
        return [
            'comments'=>$this->getComments($searchRequestId, $pageNum)
        ];
    }
