...
class ExtApiOfferController extends \app\components\ExtApiController {
...
//        }
    ...
    public function actionSave() {
        ...
        $offer->load($data,'');
        foreach(['pay_allow_bank', 'pay_allow_paypal', 'pay_allow_jugl', 'pay_allow_pod', 'show_amount','allow_contact'] as $key) {
            $offer[$key]=$offer[$key] ? 1:0;
        }
    ...
    public function actionOpenChatExpired() {
    ...
    public function actionDetails($id) {
        ...
        $data['user']=$model->user->getShortData(['rating', 'feedback_count', 'packet', 'impressum', 'agb']);
        ...
        if (!Yii::$app->request->getQueryParam('noViewBonus') && $model->user_id!=Yii::$app->user->id && $model->status==\app\models\Offer::STATUS_ACTIVE && $model->view_bonus>0 && $model->view_bonus_used+abs($model->view_bonus)<$model->view_bonus_total+1e-6) {
        ...
        if ($offer->validate()) {
            ...
            if ($offer->view_bonus_total + $offer->buy_bonus <= Yii::$app->user->identity->balance) {
                ...
                if (empty($dataOffer['images'])) {
                    $dataOffer['images']=[\app\components\Thumb::createUrl('/static/images/account/default_interest.png','offer',true)];
                }
