...
class ApiVideoController extends \app\components\ApiController {
    ...
    public function actionDetails($id) {
		...
		$video_user = $this->getUserByVideo(Yii::$app->user->identity->id, $data['video_id']);
	...
	public function actionSetVideoBalance($id){
			...
			$check = $this->getUserByVideo(Yii::$app->user->identity->id, $data['video_id']);
			...
			if($data['bonus']){
				$this->setUserByVideo(Yii::$app->user->identity->id, $data);
			}
	...
	public function getUserByVideo($userId, $videoId){
			return Yii::$app->db->createCommand("SELECT * FROM user_video WHERE user_id = ".$userId." AND video_id = '".$videoId."'")->query();	
	}
