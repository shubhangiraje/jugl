...
class ExtApiFavoritesController extends \app\components\ExtApiController {
    ...
    private function getFavoriteData($model,$interests) {
        ...
        if (count($model->files)>0) {
            $data['image']=$model->files[0]->getThumbUrl('offerMobile');
        } else {
            $data['image']=\app\components\Thumb::createUrl('/static/images/account/default_interest.png','offerMobile',true);
        }
    ...
    public function actionAdd() {
        return Yii::$app->db->transaction(function($db) {
            switch (Yii::$app->request->getBodyParams()['type']) {
                case 'search_request':
                    $isFavoriteSearchRequest = SearchRequestFavorite::find()->where(['user_id'=>Yii::$app->user->id, 'search_request_id'=>Yii::$app->request->getBodyParams()['id']])->one();
                    if(!$isFavoriteSearchRequest) {
                        try {
                    ...
                    break;
                case 'offer':
                    $isFavoriteOffer = OfferFavorite::find()->where(['user_id'=>Yii::$app->user->id, 'offer_id'=>Yii::$app->request->getBodyParams()['id']])->one();
                    if(!$isFavoriteOffer) {
                        try {
