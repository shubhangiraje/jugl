...
class ExtApiSpamReportController extends \app\components\ExtApiController {
    ...
    public function actionSave() {
        ...
        if ($data['user_id']) {
            ...
            \app\models\User::findOne(['id'=>Yii::$app->user->id])->deleteFriend($data['user_id'],false);

            $cui=\app\models\ChatUserIgnore::find()->where(['user_id'=>Yii::$app->user->id,'ignore_user_id'=>$data['user_id']])->one();
            if (!$cui) {
                $cui=new \app\models\ChatUserIgnore();
                $cui->user_id=Yii::$app->user->id;
                $cui->ignore_user_id=$data['user_id'];
                $cui->save();
            }
        }
