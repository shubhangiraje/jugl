...
class ApiManageNetworkController extends \app\components\ApiController {
    ...
    public function actionMoveDestinationList($move_id, $id=null, $pageNum=1) {
        if(!$id) {
            $id = Yii::$app->user->id;
        }
        ...
        return [
            'user'=>$user,
            'log'=>$this->getHierarchy($id, $move_id, $pageNum)
        ];
    }
    ...
    public function actionHierarchyList() {
        ...
        $filter = Yii::$app->request->getBodyParams()['filter'];
        return [
            'log'=>$this->getHierarchy($id, $move_id, $pageNum, $filter)
        ];
    }
