...
class ApiFundsLogTokenController extends \app\components\ApiController {

    public function getLog($year='', $month='', $pageNum=1, $status='all', $sort='-dt') {
        $perPage=50;

        $logQuery=BalanceTokenLog::find()->andWhere(['user_id'=>Yii::$app->user->id])
        ->from([new \yii\db\Expression('balance_token_log use index(user_id_dt)')])
