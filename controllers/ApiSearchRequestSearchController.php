...
class ApiSearchRequestSearchController extends \app\components\ApiController {
    ...
    private function search($country_ids=NULL,$filter=[],$pageNum=1, $user_id=NULL) {
        ...
        if ($country_ids) {
            $squery->andWhere(array('in','search_request.country_id',explode(',',$country_ids)));
        }
    ...
    private function initialData($user_id=null) {
        ...
        $countryList = SearchRequest::getCountryList($user_id);
    ...
    public function actionIndex() {
        ...
        return array_merge($this->initialData(),$this->search(Yii::$app->user->identity->country_id));
    }

    public function actionSearchBy($user_id) {
        return array_merge($this->initialData($user_id),$this->search($countryIds=NULL, $filter=NULL, $pageNum=NULL, $user_id));
    }
	/*nviimedia*/
